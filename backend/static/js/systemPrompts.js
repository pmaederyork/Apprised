...
const SystemPrompts = {
    ...
    // Export system prompts for backup
    export() {
        return {
            systemPrompts: this.state.systemPrompts,
            activeSystemPromptId: this.state.activeSystemPromptId,
            exportedAt: new Date().toISOString()
        };
    },

    // Import system prompts from backup
    import(data) {
        ...
        // Merge imported prompts (keeping existing ones in case of ID conflicts)
        Object.entries(data.systemPrompts).forEach(([id, prompt]) => {
            if (!this.state.systemPrompts[id]) {
                this.state.systemPrompts[id] = prompt;
            }
        });
    ...
    // Clear all system prompts (with confirmation)
    clear() {
        if (!confirm('Are you sure you want to delete all system prompts? This cannot be undone.')) {
            return false;
        }
        ...
        if (this.state.isEditingSystemPrompt) {
            this.exitEdit();
        }
